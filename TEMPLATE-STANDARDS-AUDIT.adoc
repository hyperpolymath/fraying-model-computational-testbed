...
* Session-local binding files discovered: `4` under `session/` plus `coordination.k9.ncl`
...
| target architecture
| `session/README.adoc`, `session/custom-checks.k9.ncl`, `session/local-hooks.sh`, `coordination.k9.ncl`
| exists
...
| `session/README.adoc`, `session/dispatch.sh`
...
| `session/local-hooks.sh`, `session/custom-checks.k9.ncl`
